...
# runtime stuff, global in lieu of objectification
ALT = None
M = None
Cal = None
MV = None
...
def one_shot(alt, mv, l):
    ff = get_FF(l)
    C = ballistic_coefficient(ff)
    tt = 0.0
    rg = 0.0
    global Traj
    Traj.append((alt, tt, rg, mv, l))
    while alt >= 0.0:
...
def print_projectile_configuration(args):
    ...
    if len(departure_angles) == 1:
        print " Form Factor: %.4f" % (form_factors[0])
    elif len(departure_angles) > 1:
        print " Form Factor data:"
        i = 0
        while i < len(departure_angles):
            print "  %.4fdeg: %.6f" % (math.degrees(departure_angles[i]),
                                       form_factors[i])
            i += 1
    ...
    if Max_Range:
        print "Est. max range: %.2fm at %.4fdeg" % (Max_Range[0],
                                                    math.degrees(Max_Range[1]))
...
def _max_range(alt, mv):
    ...
    l = (mid + low)/2.0
    h = (mid + high)/2.0
    (_, rg_low, _, _) = one_shot(alt, mv, l)
    (_, rg_high, _, _) = one_shot(alt, mv, h)
...
def _match_range(target_range,
    ...
    while abs(target_range - rg) > tolerance/2:
        ...
        mid = (high + low)/2.0
        rg1 = rg
        (tt, rg, iv, il) = one_shot(alt, mv, mid)
...
def match_range(args):
    ...
    (rg_max, da_max) = _max_range(ALT, MV)
    if target_range > rg_max + 1:
...
def _match_form_factor(l, tr, tol):
    clear_form_factors()
    ff = 1.0
    update_form_factors(l, ff)
    (_, rg, _, _) = one_shot(ALT, MV, l)
    count = 0
...
def match_form_factor(args):
    ...
    if args.shot:
        for shot in args.shot:
            ...
            tr = float(tr)
            (ff, l, rg, count) = _match_form_factor(da, tr, tolerance)
            shots.append((ff, l, rg, count))
    else:
        (ff, l, rg, count) = _match_form_factor(L, target_range, tolerance)
        shots.append((ff, l, rg, count))
...
def sort_form_factors(tda, tff):
    z = zip(tda, tff)
    z.sort(key=lambda k: k[0])
    tda, tff = zip(*z)
    return (list(tda), list(tff))

def load_form_factors(args):
    tda = []
    tff = []
    if 'F' not in args or len(args.F) == 0:
        tda.append(args.departure_angle)
    ...
    global departure_angles
    global form_factors
    (departure_angles, form_factors) = sort_form_factors(tda, tff)

def clear_form_factors():
    global departure_angles
    departure_angles = []
    global form_factors
    form_factors = []

def update_form_factors(da, ff):
    global departure_angles
    departure_angles.append(da)
    global form_factors
    form_factors.append(ff)
    (departure_angles, form_factors) = sort_form_factors(departure_angles, form_factors)
...
def get_FF(da):
    ...
    i = 1
    while i < len(departure_angles)-1:
        if da < departure_angles[i]:
            break
        i += 1
    da1 = departure_angles[i-1]
    da2 = departure_angles[i]
    ff1 = form_factors[i-1]
    ff2 = form_factors[i]
...
def parse_args():
    ...
    parser_mar.add_argument('-f', '--form-factor',
        action='store',
        required=True,
        type=float,
        help='Projectile form factor')
    parser_mar.add_argument('-F',
        action='append',
        required=False,
        metavar='FF,A',
        default=[],
